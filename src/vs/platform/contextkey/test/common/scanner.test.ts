...
suite('Context Key Scanner', () => {
	...
	suite('handling lexical errors', () => {
		...
		test(`foo === '`, () => {
			const input = `foo === '`;
			assert.deepStrictEqual(scan(input), ([{ type: "Str", lexeme: "foo", offset: 0 }, { type: "==", offset: 4 }, { type: "ErrorToken", offset: 8, lexeme: "'" }, { type: "EOF", offset: 9 }]));
		});
		...
		test(`foo === bar'`, () => {
			...
			assert.deepStrictEqual(r, ["Unexpected token ''' at offset 11"]);
		});
