	...
	public static deserialize(serialized: string | null | undefined): ContextKeyExpression | undefined {
		...
		if (expr === undefined) { // print a console warning
			...
			if (parser.parsingErrors.length > 0) {
				...
				parser.parsingErrors.forEach(({ message }: { message: string }) => warning.push(`${message}`, '\n'));
			}
...
export class Parser {
	...
	private _current = 0;
	private _parsingErrors: { token: Token; message: string }[] = [];
	...
	parse(input: string): ContextKeyExpression | undefined {
		...
		try {
			...
			if (!this._isAtEnd()) {
				const unexpectedToken = this._peek();
				this._parsingErrors.push({ token: this._peek(), message: `Unexpected token${unexpectedToken.lexeme ? ' \'' + unexpectedToken.lexeme + '\'' : ''} at offset ${unexpectedToken.offset}` });
	...
	private _term(): ContextKeyExpression {
		if (this._match(TokenType.Neg)) {
			...
			throw this._error(this._peek(), `Expected KEY, 'true', or 'false'`);
		}
